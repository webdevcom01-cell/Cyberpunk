    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="md:hidden border border-primary/30 hover:border-primary/50 hover:bg-primary/10 h-11 w-11 focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
      ...
      <SheetContent
        side="left"
        className="glass-sidebar w-72 p-0 scanlines"
        id="mobile-nav-menu"
        aria-label="Mobile navigation"
      >
        <SheetHeader className="flex h-16 flex-row items-center gap-3 border-b border-primary/20 px-6">
          ...
          <div className="flex flex-col">
            <SheetTitle className="text-sm font-bold text-primary terminal-glow tracking-wider">CREWAI_SYS</SheetTitle>
        ...
        <nav
          className="flex-1 space-y-1 p-4 overflow-y-auto max-h-[calc(100vh-200px)]"
          role="menu"
          ...
          {navItems.map((item, index) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "group flex items-center gap-3 rounded border px-4 py-3 text-xs font-bold tracking-wider transition-all duration-200 h-11",
        ...
        <div className="border-t border-primary/20 p-4 space-y-3" aria-label="Connection status">
          <div className="flex items-center justify-between px-2">
            <span className="text-xs font-bold text-muted-foreground tracking-wider">CONNECTION</span>
            <RealtimeStatus />
          </div>
